...
This page is about why Komodo behaves as it does. For steps, see the how-to guides; for facts, see
the reference.
...
You set how far Komodo goes on its own.

- **Auto-fix.** Komodo restarts the container as soon as it has diagnosed the problem, then verifies
  the container came back.
- **Ask first.** Komodo stops at a diagnosis and waits for a person to approve the fix.
...
Guardrails are the limits Komodo will not cross, whatever the autonomy. The agent only ever runs
...
so a sluggish model never blocks incident handling. If the model returns nothing useful, Komodo
