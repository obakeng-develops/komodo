...
Komodo is deliberately small to run. The minimum is **one always-on instance and a database** —
it doesn't matter where you host it (a VPS, Docker Compose, Fly, Kamal, your own box).

- **Run exactly one instance — don't scale it out.** Komodo is a single process and must not run
...
- **A PostgreSQL database.** Any Postgres works — bundled (Docker Compose ships one), managed, or
  ...
  Komodo creates and evolves its own tables on startup. (Local dev can use a SQLite `DATABASE_URL`.)
- **A handful of secrets**, generated by `./scripts/gen-secrets.sh`: `AUTH_SECRET` (Komodo refuses
  to start on the built-in default), `ENCRYPTION_KEY`, `INTERNAL_API_KEY`, and `COOKIE_SECURE=true`
  when served over HTTPS.
- **An LLM key is optional.** Without one, Komodo still monitors and restarts; it just skips the
  written diagnosis. Add a provider in **Settings** when you want it.
- **The executor does not run here.** Komodo restarts containers on a *watched* host by installing
  the [agent](docs/how-to/install-the-agent.md) there — the Komodo box itself needs no Docker socket.
...
> Read `docs/how-to/deploy-with-docker-compose.md` and deploy Komodo by following the steps. Check
